...
export default function Home() {
  ...
  return (
    <Layout>
      ...
      {fullScreenImage && (
        ...
        >
          <button className="absolute top-6 right-6 text-brand-main bg-brand-card hover:bg-brand-btn rounded-full w-10 h-10 flex items-center justify-center transition-all text-xl z-50">✕</button>
          <img 
            src={fullScreenImage} 
      ...
      <section className="mb-24 max-w-5xl mx-auto px-4 grid md:grid-cols-2 items-center gap-12">
        <div className="text-right order-2 md:order-1">
          ...
          المصوره السعوديـه <span className="text-brand-main font-bold">زُبـيـده عبدالعـزيـز</span> تخصصها في مجال تصوير الاعراس لتكون جزءاً مهماً في لـيلــة عمر العروسين وتوثيق التفاصيل بكل مافيها من مشاعر بإحترافيه بخدمة التصوير الفوتوغرافـي والفيديو
          </p>
        </div>
      ...
      <section className="mb-20">
        <div className="flex justify-between items-center mb-8 px-4">
          <div>
            <h3 className="text-2xl md:text-3xl font-bold text-brand-main">باقـات الاسـتـوديــو الـحـصـريـــــة</h3>
            <p className="text-brand-muted text-sm mt-1">اسحب أفقياً لاستكشاف باقـاتـنـا السبــع الفاخــرة</p>
          </div>
        </div>
  ...
  {/* استخدام متغير اللون مباشرة في الـ style لضمان التطابق التام */}
  <div 
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {portfolioPhotos.map((photo) => (
              ...
              >
                ...
                {/* تدرج الظل من الأسفل للأعلى فقط */}
<div 
...
></div>                
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="bg-brand-card text-brand-main w-12 h-12 flex items-center justify-center rounded-full backdrop-blur-sm text-xl border border-brand shadow-lg">🔍</span>
                </div>
