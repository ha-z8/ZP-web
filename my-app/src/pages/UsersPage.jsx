...
export default function UsersPage() {
  ...
  // 👈 منطق الفلترة (البحث في الاسم، اسم المستخدم، أو البريد)
    ...
    <AdminLayout>
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          <h3 className="text-lg font-bold text-purple-400">👥 إدارة حسابات العملاء والصلاحيات</h3>
          <input 
            type="text" 
            placeholder="بحث بالاسم، اسم المستخدم، أو البريد..." 
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full md:w-80 bg-slate-950 border border-slate-800 text-white px-4 py-2 rounded-xl text-sm focus:border-purple-500 focus:outline-none"
          />
        </div>

        <table className="w-full text-right text-sm text-slate-300">
          <thead className="bg-slate-950 text-slate-400 text-xs font-bold border-b border-slate-850">
            <tr><th className="p-4">بيانات العميل</th><th className="p-4">البريد</th><th className="p-4 text-center">الصلاحية</th><th className="p-4 text-center">الإجراءات</th></tr>
          </thead>
          <tbody className="divide-y divide-slate-850/60">
            {filteredUsers.map((u) => (
              <tr key={u.id} className="hover:bg-slate-900/50">
                <td className="p-4"><p className="font-bold">{u.full_name}</p><p className="text-amber-400 text-xs">@{u.username}</p></td>
                <td className="p-4">{u.email}</td>
                <td className="p-4">
                  <select value={u.role || 'user'} onChange={(e) => handleUpdateUserRole(u, e.target.value)} className="bg-slate-800 p-2 rounded text-xs">
                    <option value="user">👤 عميل</option><option value="admin">🛡️ مشرف</option>
                  </select>
                </td>
                <td className="p-4">
                  <div className="flex items-center justify-center gap-2">
                    <button onClick={() => { setEditingUser(u); setIsEditUserModalOpen(true); }} className="bg-slate-800 hover:bg-amber-500 p-2 rounded-lg transition-all border border-slate-700">📝</button>
                    <button onClick={() => handleForceChangePassword(u.id, u.full_name)} className="bg-slate-800 hover:bg-emerald-500 p-2 rounded-lg transition-all border border-slate-700">🔐</button>
                    <button onClick={() => handleSendPasswordReset(u.email)} className="bg-slate-800 hover:bg-blue-500 p-2 rounded-lg transition-all border border-slate-700">📧</button>
                    <button onClick={() => handleDeleteUser(u.id, u.full_name)} className="bg-slate-800 hover:bg-red-500 p-2 rounded-lg transition-all border border-slate-700">🗑️</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-3xl w-full max-w-lg p-8 shadow-2xl">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-4">
              <input type="text" value={editingUser.full_name} onChange={(e) => setEditingUser({...editingUser, full_name: e.target.value})} className="w-full bg-slate-900 p-3 rounded-xl" placeholder="الاسم الكامل" />
              <input type="text" value={editingUser.username} onChange={(e) => setEditingUser({...editingUser, username: e.target.value})} className="w-full bg-slate-900 p-3 rounded-xl" placeholder="اسم المستخدم" />
              <input type="email" value={editingUser.email} onChange={(e) => setEditingUser({...editingUser, email: e.target.value})} className="w-full bg-slate-900 p-3 rounded-xl" placeholder="البريد الإلكتروني" />
              <div className="flex gap-2">
                <button type="submit" className="flex-1 bg-purple-500 p-3 rounded-xl font-bold">{submittingUserEdit ? 'جاري الحفظ...' : 'حفظ التعديلات'}</button>
                <button type="button" onClick={() => setIsEditUserModalOpen(false)} className="flex-1 bg-slate-800 p-3 rounded-xl font-bold">إلغاء</button>
              </div>
            </form>
          </div>
        </div>
